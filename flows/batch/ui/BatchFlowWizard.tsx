...
import type { BatchDraft, BatchFlowRole, BatchFlowState } from '../batchFlowContract';
import { 
  BatchWizardModel, 
...
export const BatchFlowWizard: React.FC<BatchFlowWizardProps> = ({ onExit }) => {
  const layout = useDeviceLayout();
  const [model, setModel] = useState<BatchWizardModel>(createDefaultBatchWizardModel());

  const roles: BatchFlowRole[] = ["Planner", "Supervisor", "Operator"];
  ...
  const handleStepChange = (step: typeof model.step, state: BatchFlowState) => {
    setModel(m => ({ ...m, step, state }));
  };
          ...
          right={
            <div className="flex items-center gap-3">
              {model.step === "DRAFT" && (
                <>
                  <button 
                    onClick={() => setModel(createDefaultBatchWizardModel())}
                  ...
                  <button 
                    onClick={() => handleStepChange("APPROVAL", "Approved")}
                    disabled={!model.draft.batchName || !model.draft.skuCode}
              ...
              {model.step === "APPROVAL" && (
                <>
                  <button 
                    onClick={() => handleStepChange("DRAFT", "Draft")}
                  ...
                  <button 
                    onClick={() => handleStepChange("EXECUTION", "InProgress")}
              ...
              {model.step === "EXECUTION" && (
                <button 
                  onClick={() => handleStepChange("COMPLETION", "Completed")}
