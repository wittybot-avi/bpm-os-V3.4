...
import { 
  ...
  LogOut,
  Fingerprint,
  // Fix: Added missing icon imports to resolve 'Cannot find name' and shadowing errors
  Info,
  Lock
} from 'lucide-react';
...
export const DispatchWizard: React.FC<DispatchWizardProps> = ({ onExit }) => {
  const layout = useDeviceLayout();
  const [model, setModel] = useState(createDefaultDispatchWizardModel());
  ...
  // Handlers
  const handleNext = () => {
    if (model.step === "DRAFT") setModel(m => ({ ...m, step: "APPROVAL" }));
    else if (model.step === "APPROVAL") setModel(m => ({ ...m, step: "EXECUTION", state: "Approved" }));
    else if (model.step === "EXECUTION") setModel(m => ({ ...m, step: "DELIVERY", state: "Dispatched" }));
    else if (model.step === "DELIVERY") setModel(m => ({ ...m, step: "COMPLETION", state: "Delivered" }));
  };

  const handleFinalize = () => {
    setModel(m => ({ ...m, state: "Closed" }));
  };

  const handleReset = () => {
    setModel(createDefaultDispatchWizardModel());
  };
    ...
    >
      <div className="h-full flex flex-col relative">
        <div className="px-6 py-1 bg-slate-100 border-b border-slate-200 flex justify-between items-center text-[9px] font-mono text-slate-500">
           ...
           <span className="font-bold text-brand-600 uppercase">Stage S11-S14 Gate</span>
        </div>
                ...
                <div className="space-y-4">
                  <div className="space-y-1.5">
                    ...
                    <input 
                      ...
                      onChange={e => setModel(m => ({ ...m, draft: { ...m.draft, customerName: e.target.value } }))}
                    />
                  </div>
                  <div className="space-y-1.5">
                    ...
                    <div className="relative">
                      <input 
                        ...
                        onChange={e => setModel(m => ({ ...m, draft: { ...m.draft, destination: e.target.value } }))}
                      />
          ...
          {model.step === "EXECUTION" && (
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-4">
                  <div className="p-4 bg-slate-900 text-white rounded-xl border border-slate-700 flex flex-col justify-center items-center gap-3">
                    <ClipboardCheck size={32} className="text-brand-400" />
                    <div className="text-center">
                      <div className="text-sm font-bold tracking-tight">GATE PASS GENERATED</div>
              ...
              <div className="flex flex-col items-center py-10">
                ...
                <div className="mt-10 w-full max-w-md bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm">
                   ...
                   <div className="p-6 flex flex-col items-center gap-4">
                      ...
                      <p className="text-[10px] text-slate-400 italic">Code provided to the customer during dispatch authorization.</p>
                   </div>
                </div>
              </div>
            </FlowStep>
          ...
          right={
            <div className="flex items-center gap-3">
              {model.step === "DRAFT" && (
                <button 
                  onClick={handleNext}
                  disabled={!model.draft.customerName || !model.draft.destination || model.role !== 'SCM'}
