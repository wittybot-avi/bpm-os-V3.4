 ...
 * Local state implementation for V3.4 Phase B.
...
export const InboundFlowWizard: React.FC<InboundFlowWizardProps> = ({ onExit }) => {
  const layout = useDeviceLayout();
  const [model, setModel] = useState<InboundWizardModel>(createDefaultInboundWizardModel());
  ...
  const handleNextStep = () => {
    if (model.step === "RECEIPT") {
      setModel(m => ({ ...m, state: nextStateOnSerialize(), step: "SERIALIZATION" }));
    } else if (model.step === "SERIALIZATION") {
      setModel(m => ({ ...m, state: nextStateOnSubmitQc(), step: "QC" }));
    }
  };

  const handleCompleteQc = (decision: "PASS" | "FAIL" | "SCRAP") => {
    setModel(m => ({ 
      ...m, 
      state: nextStateOnQcDecision(decision), 
      step: "DISPOSITION" 
    }));
  };

  const handleReset = () => {
    setModel(createDefaultInboundWizardModel());
  };
      ...
      <div className="h-full flex flex-col relative">
        <div className="px-6 py-1 bg-slate-100 border-b border-slate-200 flex justify-between items-center text-[9px] font-mono text-slate-500">
           <div className="flex items-center gap-2">
              ...
              <span>Flow Status: <strong>{model.state}</strong></span>
           </div>
           <span className="text-slate-400 font-bold uppercase">Local Mode</span>
        </div>
        ...
        <FlowFooter 
          ...
          right={
            <div className="flex items-center gap-3">
              {model.step === "RECEIPT" && (
                <>
                  <button onClick={handleReset} className="flex items-center gap-2 px-4 py-2 text-sm font-bold text-slate-600 hover:bg-slate-100 rounded transition-all">
                    <RotateCcw size={16} /> Reset
                  </button>
                  <button 
                    onClick={handleNextStep}
                    disabled={!model.receipt.grnNumber || !model.receipt.materialCode || model.receipt.quantityReceived <= 0}
                    className="flex items-center gap-2 px-6 py-2 bg-brand-600 text-white rounded font-bold text-sm hover:bg-brand-700 disabled:opacity-50 shadow-sm"
                  >
